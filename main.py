...
from utils.util import load_videocmds
from utils.keep_alive import keep
...
async def on_application_command_error(ctx, error):
  if isinstance(error, commands.MissingRole):
    ...
    logger.error(f"Something unexpected happened {error}")
    raise error
...
async def on_ready():
  ...
  await load_videocmds(bot)

for cog_file in settings.COGS_DIR.glob("*.py"):
    if cog_file.name != "__init__.py":
        bot.load_extension(f"cogs.{cog_file.name[:-3]}")
        logger.debug(f"Loadded COG: {cog_file.name}")
